...
namespace Remotion.Web.ExecutionEngine
{
  public class WxePageInfo : WxeTemplateControlInfo, IDisposable
  {
    ...
    /// </param>
    public WxePageInfo (WxePage page)
      : base (page)
    {
    ...
    private NameValueCollection DeterminePostBackMode (HttpContext httpContext)
    {
      ...
      if (string.Equals(httpContext.Request.HttpMethod, "POST", StringComparison.InvariantCultureIgnoreCase))
        collection = httpContext.Request.Form;
      else
        collection = httpContext.Request.QueryString;

      if ((collection["__VIEWSTATE"] == null) && (collection[Page.postEventSourceID] == null))
        return null;
      else
        return collection;
    }
    ...
    private void HandlePagePreInit (object sender, EventArgs eventArgs)
    {
      ...
      if (CurrentPageStep != null)
        _page.ClientScript.RegisterHiddenField (WxePageInfo.PageTokenID, CurrentPageStep.PageToken);
    ...
    private void Form_LoadPostData (object sender, EventArgs e)
    {
      ...
      NameValueCollection postBackCollection = ((IWxePage)_page).GetPostBackCollection ();
    ...
    public void OnPreRenderComplete ()
    {
      ...
      _page.ClientScript.RegisterHiddenField (WxeHandler.Parameters.WxeFunctionToken, wxeContext.FunctionToken);
      _page.ClientScript.RegisterHiddenField (WxePageInfo.ReturningTokenID, null);
      int nextPostBackID = wxeContext.PostBackID + 1;
       _page.ClientScript.RegisterHiddenField (WxePageInfo.PostBackSequenceNumberID, nextPostBackID.ToString ());

      _page.ClientScript.RegisterClientScriptBlock (typeof (WxePageInfo),  "wxeDoSubmit",
      ...
      _page.ClientScript.RegisterClientScriptBlock (typeof (WxePageInfo), "wxeDoPostBack",
